...
public class MaxConsecutiveAnswers {
    ...
    public int maxConsecutiveAnswers(String answerKey, int k) {
        ...
        int Tcounts = getLongestOnes(chars, k, 'T');
        int Fcounts = getLongestOnes(chars, k, 'F');
        return Math.max(Tcounts, Fcounts);
    }

    private int getLongestOnes(char[] chars, int k, char condition) {
        ...
        // #1004 虫取
        while (right < length) {
            if (chars[right] == condition) {
                conditionNums++;
            }

            while (conditionNums > k) {
                if (chars[left] == condition) {
                    conditionNums--;
                }
                left++;
            }
